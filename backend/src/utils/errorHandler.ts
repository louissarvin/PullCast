  ...
  try {
    ...
    ((prismaQuery as any).errorLog as { create: (args: { data: typeof errorLogData }) => Promise<unknown> })
      ...
      .catch((dbError: unknown) => {
        console.error('Failed to log error to database:', redactSecrets(dbError));
      });

    // Log to console for development. Secret-redact the original error so
    // a Discord bot token or pg URL surfaced via a third-party SDK does not
    // land in the shared log stream.
    // 404s (NOT_FOUND) are expected client-driven states — log them as info
    // instead of error so the ops signal stays clean.
    const logFn = statusCode === 404 ? console.info : console.error
    logFn(`[${errorCode}] ${message}`, {
      statusCode,
      userId,
      path: requestInfo.path,
      method: requestInfo.method,
      originalError: originalError ? redactSecrets(originalError.message) : undefined,
      stack: originalError ? redactSecrets(originalError.stack ?? '') : undefined,
    });
    ...
    console.error('Error in error handler:', redactSecrets(handlerError));
